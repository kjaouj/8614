...
def stub_reply(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = "Bonjour, nous avons bien reçu votre message et revenons vers vous rapidement."  # TODO: message minimal (sera remplacé plus tard)
...
def stub_ask_clarification(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = ("Pouvez-vous préciser votre demande et fournir les informations manquantes "
        "(référence, date ou contexte) afin que nous puissions vous aider ?")  # TODO: 1-2 questions génériques (sera remplacé plus tard)
...
def stub_escalate(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "handoff_human",
        "summary": "Demande à risque ou bloquante nécessitant une prise en charge humaine.",   # TODO: résumé court pour escalade
    })
...
def stub_ignore(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "ignore",
        "reason": "Message purement informatif ou hors périmètre.",  # TODO: raison courte (ex: hors périmètre)
    })
